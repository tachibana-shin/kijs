...
```
  ...
  /**
    ...
    *
    * **Solution**: Instead of `.click(fn)` use `.on("click", fn)`. Instead of `.click()` use `.trigger("click")`.
    * @example ​ ````Show the event object when a key is pressed in the input. Note: This demo relies on a simple $.print() plugin (https:
```html
<!doctype html>
<html lang="en">
<head>
...
```
   ...
   * @see \`{@link https:
   * @since 1.4
   * @deprecated ​ Deprecated since 3.0. See \`{@link https:
   *
